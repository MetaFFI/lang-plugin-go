...
func createMetaFFITypeInfoArray(paramsTypes []IDL.MetaFFITypeInfo) *C.struct_metaffi_type_info {
	...
	for i, v := range paramsTypes {
		...
		if v.Alias != "" {
			...
			metaffi.alias_length = C.ulong(len(v.Alias))
		} else {
			metaffi.alias = nil
...
func FromGoToCDT(input interface{}, pdata unsafe.Pointer, i int) {
	...
	case []MetaFFIHandle:
		...
		*out_input_dimensions_lengths = C.ulong(len(input.([]MetaFFIHandle)))

		out_input := (*C.struct_cdt_metaffi_handle)(C.malloc(C.ulong(len(input.([]MetaFFIHandle))) * (C.sizeof_struct_cdt_metaffi_handle)))
